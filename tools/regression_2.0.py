...
def calculate_extended_regression():
    ...
    # 准备自变量矩阵 X
    # 包括: 常数项(用于a0), Time of Flight, delta_t, delta_rh, delta_p
    X = np.column_stack([
...
def main():
    try:
        ...
        print(f"b0 (温度系数): {b0:.6f} mm/°C")
        print(f"b1 (湿度系数): {b1:.6f} mm")
        print(f"b2 (压力系数): {b2:.6f} mm/kPa")

        print(f"\n回归模型:")
        print(f"d = {a0:.6f} + ({a1:.6f}) * (Time of Flight) + ({b0:.6f}) * delta_t + ({b1:.6f}) * delta_rh + ({b2:.6f}) * delta_p")
        ...
        # 输出每个数据点的预测和残差
        print(f"\n详细结果:")
        print("真实距离(mm) | 预测距离(mm) | 残差(mm)")
        print("-" * 40)
        for i in range(len(y)):
            print(f"{y[i]:.1f}        | {y_pred[i]:.1f}        | {residuals[i]:.2f}")
